...
- **Création** : Utilisateur → Mot de passe → Date d'expiration (`YYYY-MM-DD HH:MM`)
- **Gestion** : Modifier mot de passe, prolonger expiration, supprimer
...
- **Création** : User → Pass → Expiration → **Limite connexions max** → **Quota data (MB)**
...
*(Le script vous demandera uniquement votre Token Telegram et le fuseau horaire).*
